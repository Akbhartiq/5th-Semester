...
compound_statement          : '{' M3 block_item_list_opt '}' { //M3 is used to denote if we entering a new block
                                if(func_flag==2)func_flag=0;
                                $$ = $3;
                            }
                            ;

//If we are again entering a new block the but have currently seen the function declartion i.e it will be function defination
M3                          : {
                                if(func_flag==1)
                                {
                                    func_flag=2;
...
block_item                  : declaration {
                            ...
                            | statement {
                                $$=$1;
                                global_attr_node=$$;//Purpose go global_attr_node is to check if the GOTO labels have been generated for a expression or not
                            }
                            ;
...
//M6: Get new nested st for a new block
//M12: To pop the nested ST
//M13: for generateing Instr
//M19: To generate the Instr
//Mostly all of M are used for getting Intermediate Instr in quad_index
...
//M9: Get new nested st for a new block
//M19: To check if the GOTO for the expression has been generated
//M13: To get the next instruction index
//M10: Entering a new block
//M12: To pop the nested ST
...
//To jump at the condition checking
M18                         :{
                                $$=create_att_node(NULL, NULL, 0, 0, NULL, NULL, NULL, NULL, 0, 0);
...
primary_expression          : IDENTIFIER {
                                ...
                                if(temp) {
                                    ...
                                    {
                                        $$=create_att_node(temp->name,"function",0,0,temp->hierarchical_dt,NULL,NULL,NULL,temp->size,global_offset);//Offset-Doesn't matter now
                                        global_attr_node=$$;
                                    }
                                    else
                                    {
                                        int is_array_flag=is_array(temp->hierarchical_dt);
                                        $$=create_att_node(temp->name,temp->name,0,0,temp->hierarchical_dt,NULL,NULL,NULL,temp->size,global_offset);//Offset-Doesn't matter now
                                        global_attr_node=$$;
                                    }
                                }
                                else {
...
postfix_expression          : primary_expression {
                            ...
                            | postfix_expression '[' expression ']' {
                                ...
                                char buffer[20];
                                sprintf(buffer, "%d", $1->type->sub_type->width); // Convert integer to string
                                quad_array = emit("*",$3->addr, buffer, temp);
                                if ($1->isArray) {
                                    quad_array = emit("+", temp, $1->addr, temp);
                                }
                                if ($1->type->sub_type) {
                                    data_type* temp_dt = create_data_type($1->type->sub_type->type, $1->type->sub_type->width, $1->type->sub_type->num_of_ele, $1->type->sub_type->sub_type);
                            ...
                            | postfix_expression '(' M4 argument_expression_list_opt ')' {
                                ...
                                quad_array=emit("call", $1->addr, param_counter_str, temp_Ret_val);
                                param_counter = top_param();
                                pop_param();
                                //Search for the $1->addr in the symbol table
                                symbolTable temp_node = search_in_st(global_st, $1->addr);
                                ...
                                $$->addr=temp_Ret_val;
                                $$->iftemp=1;
                                $$->type=create_data_type(temp_type, $$->width, 1, NULL);
                                global_attr_node=$1;
                            ...
                            ;
//counter the number of param to call the function
M4                          : /* epsilon */ {   
...
argument_expression_list    : assignment_expression {
                            ...
                            | argument_expression_list ',' M5 assignment_expression {
                                // emitting TAC irrespective of identifier, temp_var, constant, i.e. not doing t1 = a, param t1
                                param_counter++;
                                quad_array = emit("param", $4->addr, NULL, NULL);
...
unary_expression            : postfix_expression {
                            if ($1->isArray) {
                                char* temp = gen_temp_var();
                                quad_array = emit("[]=", $1->array, $1->addr, temp);
                                global_st = add_in_st(global_st, temp, $1->type->type, $1->type, NULL, SIZEOF_INT, global_offset, NULL);
                                $$ = create_att_node(temp, $1->array, 1, 1, $1->type, NULL, NULL, NULL, SIZEOF_INT, global_offset);
                                global_offset += SIZEOF_INT;
                            }
                            else {
                                $$=$1;
...
//To finally generate the postfix expression
M5                          : /* epsilon */ {
                                perform_postfix();
                            }
                            ;
...
init_declarator             : declarator {
                            ...
                            | declarator '=' initializer {
                                ...
                                if ($3->iftemp == 0) {
                                    ...
                                    quad_array = emit("=", $3->addr, NULL, temp_var);
                                    //Add the temp_var in the symbol-Table
                                    global_st = add_in_st(global_st, temp_var, $3->type->type, NULL, NULL, $3->width, global_offset, NULL);
                                    global_offset += $3->width;
...
direct_declarator           : IDENTIFIER {
                                ...
                                if(func_flag==1)
                                {
                                    global_st = add_in_st(global_st, $1->addr, ptr_type, temp, NULL,curr_width, global_offset, NULL);
                            ...
                            | direct_declarator '[' assignment_expression ']' {
                                ...
                                $$=create_att_node($1->addr,$1->addr,1,0,NULL,NULL,NULL,NULL,expr_val*$1->width,global_offset);
